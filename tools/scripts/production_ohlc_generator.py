...
def clear_ohlc_tables():
    ...
    # We don't actually drop tables anymore, just clear the data for the specific symbol
    logger.info("Skipping table drop - we now append to existing tables")
    pass
...
def generate_timeframe_candles(symbol, timeframe, start_date, end_date):
    ...
    if timeframe == "1d":
        # For daily candles, we need a direct approach with QuestDB's SAMPLE BY 1d ALIGN TO CALENDAR
        # We'll filter directly in the query to exclude Sunday timestamps before 22:00 UTC
        query = f"""
...
def generate_all_timeframes(symbol, start_date, end_date):
    ...
    # Handle daily candles separately
    if success:
        if not generate_timeframe_candles(symbol, "1d", start_date, end_date):
            logger.error(f"Failed to generate 1d candles")
            success = False
